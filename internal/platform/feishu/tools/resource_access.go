...
func docsResourceAccessSpec() tooltypes.Spec {
	return tooltypes.Spec{
		Name:        ResourceAccessToolName,
		Description: "Verify or request read/write access to one exact Feishu file or folder for the trusted current user and chat. Call this when a protected Feishu tool returns status=resource_authorization_required, using that error's resource_type, resource_token, and permission. Bot-owned resources and existing valid grants return granted immediately. When no sufficient local capability is recorded, LingoBridge first performs a read-only Feishu permission check; a manually granted Bot/group permission is recorded and avoids OAuth, but the requester must still choose either a temporary 10–60 minute grant or a permanent grant in the card. once_duration_minutes is the model's suggested temporary window and is shown to the user. write also satisfies read, while read never satisfies write. Only when Feishu-side permission is actually missing does LingoBridge reuse the requester's encrypted OAuth credential or update the same card with Feishu's official OAuth handoff. Use resource_token=bot_root for the Bot root or chat_default_folder for the current chat's default Bot folder.",
