...
func docsFolderCreateSpec() tooltypes.Spec {
	return tooltypes.Spec{
		...
		Description: "Create a Bot-owned Feishu folder under the Bot root or another Bot-owned folder bound to the current trusted chat, then grant the chat or private-chat user full access. The tool checks folder/write access from trusted context before creation and returns resource_authorization_required if it is missing; it does not send a resource card itself. This operation has no separate operation-approval card. If creation fails or returns partial/outcome_unknown, do not submit another create request for the same folder. Use feishu_docs_folder_list as the read-only check and accept the folder as existing only when exactly one matching folder is found. request_id mode only recovers the already-ledgered workflow or sharing completion, is not a new same-folder create attempt, and never repeats the Feishu create API.",
